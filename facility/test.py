...
class Solution():
    ...
    capacity = np.zeros(N)
    fixedCosts = np.zeros(N)
    for i in range(N):
        capacity[i] = facilities[i].capacity
        fixedCosts[i] = facilities[i].setup_cost
    
    demand = np.zeros(M)

    for j in range(M):
        demand[j] = customers[j].demand
    
    transCosts = np.zeros((M, N))

    transCosts = [[length(facilities[i].location, customers[j].location) for i in range(N)] for j in range(M)]
